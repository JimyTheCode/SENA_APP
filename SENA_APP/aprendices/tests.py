...
class AprendizFormTest(TestCase):
    ...
    def test_documento_con_letras_es_invalido(self):
        ...
        self.assertIn("solo", str(form.errors["documento_identidad"]).lower())
...
class AprendizViewTest(AprendizTestBase):
    ...
    def test_detalle_aprendiz_existente_responde_200(self):
        url = reverse(
            "aprendices:detalle_aprendiz", kwargs={"id_aprendiz": self.aprendiz.id}
        )
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Pepito")
    ...
    def test_editar_aprendiz_actualiza_datos(self):
        url = reverse(
            "aprendices:editar_aprendiz", kwargs={"aprendiz_id": self.aprendiz.id}
        )
        datos_actualizados = {
            "documento_identidad": "1234567890",
            "nombre": "Juan Carlos",
    ...
    def test_eliminar_aprendiz_lo_borra_de_la_bd(self):
        ...
        url = reverse("aprendices:eliminar_aprendiz", kwargs={"aprendiz_id": aprendiz_id})
